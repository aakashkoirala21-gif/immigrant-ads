...
export const getAppointmentsForUser = async (
  ...
  const schedules = await Schedule.findAll({
    ...
    include: [
      ...
      },
      {
        ...
        include: [
          {
            model: User, // fetch all, sanitize manually
          },
        ],
      },
      {
  ...
  return schedules.map((s) => {
    ...
    const professional = scheduleJson.professional;

    // sanitize sensitive fields from professional.User
    const safeUser = professional?.User
      ? {
          id: professional.User.id,
          full_name: professional.User.full_name,
          email: professional.User.email,
          profile_picture_url: professional.User.profile_picture_url,
          role: professional.User.role,
          location: professional.User.location, // ✅ keep location
          // ❌ omit phone, password_hash, reset tokens
        }
      : null;
    ...
    return {
      ...
      professional_details: {
        ...
        is_paid: professional?.is_paid,
        approved: professional?.approved,
        created_at: professional?.created_at,
        updated_at: professional?.updated_at,
        user: safeUser, // ✅ safe version of Professional.User
      },
      appointment_details: {
