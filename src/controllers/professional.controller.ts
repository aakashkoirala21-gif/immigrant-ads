...
export async function listProfessionals(req: Request, res: Response): Promise<any> {
  try {
    ...
    return sendResponse({ res, message: 'Professionals listed', data: professionals });
  } catch (error: any) {
    return sendResponse({ res, statusCode: 500, success: false, message: error.message || 'Failed to list professionals' });
  }
}
